...
def diamond_pts(n):
    ...
    for i in range(n):
        t = i / n
        if t < 0.25:
            pts.append(A + (B - A) * (t * 4))
        elif t < 0.5:
            pts.append(B + (C - B) * ((t - 0.25) * 4))
        elif t < 0.75:
            pts.append(C + (D - C) * ((t - 0.5) * 4))
        else:
            pts.append(D + (A - D) * ((t - 0.75) * 4))
    return np.array(pts)
    ...
    def smooth_grid(self, w):
        ...
        for i in range(1, self.h-1):
            for j in range(1, self.w-1):
                new_w[i, j] = (0.5 * w[i, j] + 0.125 * (w[i-1, j] + w[i+1, j] + w[i, j-1] + w[i, j+1]))
        return new_w
# GUI
class App:
